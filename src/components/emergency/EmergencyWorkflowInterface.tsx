...
import { WatermarkFeature } from "@/components/documents/WatermarkFeature";
  ...
  const handleEmergencySubmit = async () => {
    ...
    if (isParallel) {
      ...
      selectedRecipients.forEach((recipientId) => {
        ...
        workflowSteps.push({
          ...
          status: 'current', // ALL current in parallel
    ...
    if (hasEscalation) {
      ...
      import('@/services/EscalationService').then(({ escalationService }) => {
        const timeoutMs = (escalationService.constructor as any).timeUnitToMs(
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <Button
              ...
              size="lg"
              className={`w-full sm:w-auto font-bold ${isEmergencyMode ? 'animate-pulse shadow-glow' : ''}`}
            >
              {isEmergencyMode ? (
                <>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="border-destructive focus:ring-destructive text-base sm:text-sm"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger className="border-destructive focus:ring-destructive text-base sm:text-sm">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 pt-4">
              ...
              <Button
                ...
                variant="destructive"
                className="font-bold animate-pulse w-full sm:w-auto"
              >
                    ...
                    <Card key={recipientId} className="overflow-hidden">
                      ...
                      {isOpen && (
                        <div className="p-4 border-t space-y-3">
                          ...
                          <div className="space-y-2"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Smartphone className="w-4 h-4 text-muted-foreground" /><span className="text-sm font-medium">Push</span></div><Switch checked={recipientSettings.pushNotifications} onCheckedChange={(checked) => setRecipientNotifications({ ...recipientNotifications, [recipientId]: { ...recipientSettings, pushNotifications: checked } })} /></div>{recipientSettings.pushNotifications && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 ml-0 sm:ml-6"><Input type="number" value={recipientSettings.pushInterval} onChange={(e) => setRecipientNotifications({ ...recipientNotifications, [recipientId]: { ...recipientSettings, pushInterval: e.target.value } })} min={1} className="h-8 text-base sm:text-sm" /><Select value={recipientSettings.pushUnit} onValueChange={(value: any) => setRecipientNotifications({ ...recipientNotifications, [recipientId]: { ...recipientSettings, pushUnit: value } })}><SelectTrigger className="h-8 text-base sm:text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="seconds">Seconds</SelectItem><SelectItem value="minutes">Minutes</SelectItem><SelectItem value="hours">Hours</SelectItem><SelectItem value="days">Days</SelectItem><SelectItem value="weeks">Weeks</SelectItem><SelectItem value="months">Months</SelectItem></SelectContent></Select></div>)}</div>
                          <div className="space-y-2"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><MessageCircle className="w-4 h-4 text-green-600" /><span className="text-sm font-medium">WhatsApp</span></div><Switch checked={recipientSettings.whatsappNotifications} onCheckedChange={(checked) => setRecipientNotifications({ ...recipientNotifications, [recipientId]: { ...recipientSettings, whatsappNotifications: checked } })} /></div>{recipientSettings.whatsappNotifications && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 ml-0 sm:ml-6"><Input type="number" value={recipientSettings.whatsappInterval} onChange={(e) => setRecipientNotifications({ ...recipientNotifications, [recipientId]: { ...recipientSettings, whatsappInterval: e.target.value } })} min={1} className="h-8 text-base sm:text-sm" /><Select value={recipientSettings.whatsappUnit} onValueChange={(value: any) => setRecipientNotifications({ ...recipientNotifications, [recipientId]: { ...recipientSettings, whatsappUnit: value } })}><SelectTrigger className="h-8 text-base sm:text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="seconds">Seconds</SelectItem><SelectItem value="minutes">Minutes</SelectItem><SelectItem value="hours">Hours</SelectItem><SelectItem value="days">Days</SelectItem><SelectItem value="weeks">Weeks</SelectItem><SelectItem value="months">Months</SelectItem></SelectContent></Select></div>)}</div>
                        </div>
                      )}
                    </Card>
                  );
