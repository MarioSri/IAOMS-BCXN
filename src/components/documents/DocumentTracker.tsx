...
export const DocumentTracker: React.FC<DocumentTrackerProps> = ({ userRole, userName, onViewFile, onViewFiles }) => {
  ...
  // Use real-time documents with normalization to match local Document interface
  useEffect(() => {
    if (trackDocuments && trackDocuments.length > 0) {
      console.log('🔄 [DocumentTracker] Normalizing trackDocuments:', trackDocuments.length);
      const normalizedDocs = (trackDocuments as any[]).map(doc => {
        // If it's already a Document with mandatory fields, return it
        ...
        return {
          ...
          submittedBy: doc.submittedBy || doc.submitter || doc.sender_id || 'Unknown',
          submittedDate: doc.submittedDate || (doc.created_at ? doc.created_at.split('T')[0] : new Date().toISOString().split('T')[0]),
          ...
          requiresSignature: doc.requiresSignature ?? (doc.type !== 'Report'),
    ...
    } else if (trackDocuments && trackDocuments.length === 0 && submittedDocuments.length > 0) {
      // Only clear if we really intentionally got an empty array back
      // and aren't just waiting for initialization
    }
  }, [trackDocuments]);
