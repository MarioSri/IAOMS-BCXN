                ...
                <CardContent>
                  ...
                  <div className="grid grid-cols-7 gap-1 sm:gap-2">
                    {generateCalendarDays().map((day) => (
                      <div
                        key={day.date}
                        className={`p-1 sm:p-2 rounded-lg cursor-pointer transition-all hover:bg-accent flex flex-col items-center justify-start min-h-[2.5rem] sm:min-h-[3rem] ${day.isToday ? 'bg-primary text-primary-foreground' :
                        ...
                        {day.meetings.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {day.meetings.slice(0, 2).map((meeting, idx) => (
                              <Tooltip key={idx}>
                                <TooltipTrigger>
                                  <div className={`w-2 h-2 rounded-full ${meeting.status === 'confirmed' ? 'bg-green-500' :
